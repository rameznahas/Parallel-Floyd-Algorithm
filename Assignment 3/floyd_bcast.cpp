...
void parallel_floyds_bcast(int*& g, int p_row, int p_col, int n, int count) {
	for (int k = 0; k < n; ++k) {
		int row_buff;
		int* col_buff = new int[count];

		communication(row_buff, col_buff, g, p_row, p_col, k, count);
...
int main(int argc, char** argv) {
	...
	clock_t start = clock();
	parallel_floyds_bcast(g, p_coord.first, p_coord.second, n, count);
